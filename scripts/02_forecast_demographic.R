...
data <- read_csv("data_raw/percent_white_by_tract_raw.csv")
...
fit_arima_safe <- function(ts) {
  tryCatch(
    model(ts, arima = ARIMA(percent_white ~ pdq(p = 1:3, d = 1, q = 0:3))),
    error = function(e) {
      message("Fallback for GEOID: ", unique(ts$GEOID))
      tryCatch(
        model(ts, arima = ARIMA(percent_white ~ pdq(1, 1, 0))),
...
forecast_clean <- forecast_data %>%
  ...
  transmute(
    ...
    NAME = NA_character_,
    year,
    percent_white = pmin(100, pmax(0, .mean))
  )

write_csv(forecast_clean, "outputs/forecast_percent_white_by_tract.csv")
write_xlsx(forecast_clean, "outputs/forecast_percent_white_by_tract.xlsx")
