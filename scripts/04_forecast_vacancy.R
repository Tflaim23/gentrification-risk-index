...
data <- read_csv("data_raw/vacancy_rate_by_tract_raw.csv") %>%
...
fit_arima_safe <- function(ts) {
  tryCatch(
    model(ts, arima = ARIMA(vacancy_rate ~ pdq(p = 1:3, d = 1, q = 0:3))),
    error = function(e) {
      message("Fallback for GEOID: ", unique(ts$GEOID))
      tryCatch(
        model(ts, arima = ARIMA(vacancy_rate ~ pdq(1, 1, 0))),
...
forecast_clean <- forecast_data %>%
  ...
  transmute(
    ...
    NAME = NA_character_,
    year,
    vacancy_rate_forecast = pmin(100, pmax(0, .mean))
  )

write_csv(forecast_clean, "outputs/forecast_vacancy_rate_by_tract.csv")
write_xlsx(forecast_clean, "outputs/forecast_vacancy_rate_by_tract.xlsx")
